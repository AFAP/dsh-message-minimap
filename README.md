...
| 点击早期刻度没跳转 | 0.4.3 起按消息 id 精确跳转：插件会先自动向上翻页（聊天顶部出现"加载中"），跨越多页（含不含用户消息的页）直到目标消息出现；若历史全部加载完仍不存在（分岔/已删除消息），会安静地停在原地，不会跳偏。旧版本的位置对齐（`DOM 下标 = 全量下标 − 差值`）在 API 时序抖动或插队消息时会跳偏，升级即可。 |
