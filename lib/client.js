  ...
  factory: function (require) {
    ...
    function UserMessageMinimap(props) {
      ...
      function jumpToMessage(index) {
        ...
        var pages = 0;
        var step = function () {
          if (pages >= MAX_JUMP_PAGES) { jumpingRef.current = false; return; }
          var container = containerRef.current;
          if (container === null || !container.isConnected) { jumpingRef.current = false; return; }
          var node = findFlowNode(container, targetKey);
          if (node !== null) {
            jumpingRef.current = false;
            scrollToFlowNode(node, true);
            return;
          }
          if (container.querySelector(OLDER_SELECTOR) === null) {
            // The pager is gone and the target is still absent — the message
            // is not in this session's history. Give up quietly.
            jumpingRef.current = false;
            return;
          }
          ...
          var poll = setInterval(function () {
            ...
            if (now !== before || waited >= LOAD_WAIT_MS) {
              clearInterval(poll);
              if (now === before) {
                // One page cycle produced no new DOM node. If the pager is
                // still there, try the next page; otherwise give up quietly.
                if (el !== null && el.isConnected && el.querySelector(OLDER_SELECTOR) !== null && waited >= LOAD_WAIT_MS) {
                  pages++;
                  step();
                  return;
                }
