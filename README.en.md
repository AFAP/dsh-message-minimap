...
| Clicking an early tick doesn't jump | Since 0.4.3 the jump is keyed by message id: the plugin pages up (re-triggering the pager every second, crossing pages that contain no user messages) until the target appears; if history is exhausted and the message doesn't exist (forked/deleted), it stops in place rather than jumping somewhere wrong. On give-up the Console logs a `[dsh-message-minimap]` warning — set `localStorage.dmm.debug=1`, reload and reproduce to capture the full trace. |
